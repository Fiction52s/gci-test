...
#[unsafe(no_mangle)]
pub extern "C" fn  create_workout_gci(path_ptr: *const c_char) {
    ...
    let path_str = match c_str.to_str() {
        Ok(s) => s,
        Err(e) => {
            eprintln!("Invalid UTF-8 path string: {}", e);
            return;
        }
    };

    match encode_workouts_to_bytes(path_str) {
        Ok(encoded_data) => {
            if let Err(e) = small_create_blank_replay(&encoded_data) {
                eprintln!("Failed to create minimal GCI replay");
            }
        }
        Err(e) => {
            eprintln!("Failed to encode workout JSON: {}", e);
        }
    }
}
...
//     // Use "workout.json" if no argument is given
//     let json_path = if args.len() < 2 {
//         println!("No input file provided. Using default: workout.json");
//         "workout.json"
//     } else {
//         &args[1]
//     };

//     let c_string = CString::new(json_path)?;
//     let c_ptr = c_string.as_ptr();

//     unsafe {
//         create_workout_gci(c_ptr);
//     }
